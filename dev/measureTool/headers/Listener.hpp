...
class Listener{
        ...
        struct traveler *_messager; //la structure dans la qu'elle stoquer les résultats
        pid_t _child;               //le pid du processus a surveiller (celui dans le quel tourne le code a mesurer)
        bool _error;                //si true, une erreur est survenue dans l'exécution
        std::string _errorMessage;  //le message d'erreur associé a _error
        unsigned long int *_dieCount; //un tableau qui compte le nombre de die pour chaque package cpu
        double **_firstE, **_secondE, **_maxE; //tableaux d'énèrgie, tableau[x][y], x==package ,y==die
        raplcap _capDivice;         //raplcap object
        int _packagesCount;         //nombre de cpu packages
        void start();               //lance l'exécution
        void cleanUp();             //détruit les tableau dynamiques
    public:
        ...
        /**
         * retourne le message d'erreur a afficher (si isError est true)
         */
        std::string getErrorMessage();
        
        /**
         * Launch le Listener dans un nouveau thread (il doit s'exécuter en même temps que le programme mesurer).
         * retourne quand le thread est lancer
         */
        static void launch(Listener &toLaunch, pid_t child);
        
        /**
         * Stop un Listener lancer dans un autre thread.
         * retourne quand le thread est stopper
         */
